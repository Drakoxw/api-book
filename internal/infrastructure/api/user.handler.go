...
func (uh *UserHandler) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "no existe el usuario", http.StatusNotFound)
		return
	}
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("Error actualizar el usuario: %v", err), http.StatusInternalServerError)
		return
	}
...
func (uh *UserHandler) GetUserId(w http.ResponseWriter, r *http.Request) {
	...
	idNum, err := strconv.Atoi(id)
	if err != nil {
		http.Error(w, fmt.Sprintf("Error al obtener el id: %v", err), http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("Error procesar los datos del usuario: %v", err), http.StatusInternalServerError)
	}
	w.Header().Set("Content-Type", "application/json")
	w.Write(jData)
